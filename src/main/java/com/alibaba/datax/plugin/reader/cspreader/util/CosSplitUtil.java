...
import com.alibaba.datax.plugin.reader.cspreader.CosInputStream;
 ...
 */
public class CosSplitUtil extends UnstructuredSplitUtil {
    ...
    public CosSplitUtil(COSClient ossClient, String bucketName) {
        super(false);
    ...
    public Long getFileTotalLength(String filePath) {
        ...
        COSObject ossObject = this.ossClient.getObject(getObjectRequest);
        return ossObject.getObjectMetadata().getContentLength();
    }

    @Override
    public InputStream getFileInputStream(StartEndPair startEndPair) {
        InputStream inputStream = new CosInputStream(this.ossClient, this.bucketName, startEndPair.getFilePath(),
    ...
    public List<Configuration> getSplitedConfigurations(Configuration originConfiguration, List<Pair<String, Long>> objKeySizePares,
        ...
        this.balanceThreshold = originConfiguration.getDouble(com.alibaba.datax.plugin.reader.cspreader.Key.BALANCE_THRESHOLD, 10.0);
...
class Group {
     ...
     * */
    FileBlock split(Long splitLen, COSClient ossClient, String ossBucketName) {
        ...
        if (bigBlock.getSize() <= CosSplitUtil.SINGLE_FILE_SPLIT_THRESHOLD_IN_SIZE) {
            return null;
        }
        ...
        if (splitLen >= bigBlock.getSize()) {
            ...
            InputStream inputStream = new CosInputStream(ossClient, ossBucketName, outBlock.getObjName(), outBlock.getEndOffset(), originalEnd);
