package com.alibaba.datax.plugin.reader.cspreader;

import com.alibaba.datax.common.exception.DataXException;
import com.alibaba.datax.common.plugin.RecordSender;
import com.alibaba.datax.common.spi.Reader;
import com.alibaba.datax.common.util.Configuration;
import com.alibaba.datax.plugin.reader.cspreader.util.CosSplitUtil;
import com.alibaba.datax.plugin.reader.cspreader.util.CosUtil;
import com.alibaba.datax.plugin.reader.cspreader.util.HdfsParquetUtil;
...
public class CosReader extends Reader {
    public static class Job extends Reader.Job {
        ...
        private COSClient ossClient = null;
        ...
        private List<Pair<String, Long>> objectSizePairs = new ArrayList<>(); /*用于任务切分的依据*/
        ...
        private void basicValidateParameter() {
            ...
            if (StringUtils.isBlank(endpoint)) {
                throw DataXException.asDataXException(
                        CosReaderErrorCode.CONFIG_INVALID_EXCEPTION, "invalid endpoint");
            }

            accessId = this.readerOriginConfig.getString(Key.ACCESSID);
            if (StringUtils.isBlank(accessId)) {
                throw DataXException.asDataXException(
                        CosReaderErrorCode.CONFIG_INVALID_EXCEPTION,
                        "invalid accessId");
            }
        ...
        private void validate() {
            ...
            ossClient = CosUtil.initOssClient(this.readerOriginConfig);
...
//        }
        ...
        public void prepare() {
            ...
            // 将每个单独的 object 作为一个 slice
            List<String> originObjects = readerOriginConfig.getList(Key.OBJECT, String.class);
//            final int splitNum = originObjects.size() / 1000;
//            LOG.debug("split num:" + splitNum);
            if (originObjects.size() <= 1000) {
                this.objectSizePairs = parseOriginObjectSizePairs(readerOriginConfig.getList(Key.OBJECT, String.class));
                this.objects = parseOriginObjects(readerOriginConfig.getList(Key.OBJECT, String.class));
                UnstructuredStorageReaderUtil.setSourceFileName(readerOriginConfig, this.objects);
                UnstructuredStorageReaderUtil.setSourceFile(readerOriginConfig, this.objects);
            } else {
                List<List<String>> objectSplit = Lists.partition(originObjects, 500);
                LOG.debug("objectSplit = " + objectSplit.size());
                LOG.debug("originObjects.size() = " + originObjects.size());
                for (List<String> object : objectSplit) {
                    LOG.debug("object.size() = " + object.size());
                    LOG.debug("object list" + JSON.toJSONString(object));
                    this.objectSizePairs.addAll(parseOriginObjectSizePairs(object));
                    this.objects.addAll(parseOriginObjects(object));
                }
            }
            UnstructuredStorageReaderUtil.setSourceFileName(readerOriginConfig, this.objects);
            UnstructuredStorageReaderUtil.setSourceFile(readerOriginConfig, this.objects);
        ...
        public List<Configuration> split(int adviceNumber) {
            ...
            if (0 == objects.size() && this.successOnNoObject) {
                ...
                throw DataXException.asDataXException(
                        CosReaderErrorCode.EMPTY_BUCKET_EXCEPTION,
             ...
             * 这里抽出一个方法来判断是否需要分片
             * */
            CosSplitUtil ossFileSplit = new CosSplitUtil(this.ossClient, this.bucket);
        ...
        private List<Pair<String, Long>> parseOriginObjectSizePairs(List<String> originObjects) {
            ...
            for (String object : originObjects) {
                ...
                if (firstMetaChar != -1) {
                    LOG.debug("parseOriginObjectSizePairs():-1");
                    ...
                    LOG.debug("parseOriginObjectSizePairs():else");
                    try {
                        //ossClient.getObject(bucket, object);
                        ObjectMetadata objMeta = ossClient.getObjectMetadata(bucket, object);
                        //
                        LOG.info("object key:{},size:{}", object, objMeta.getContentLength());
//                        parsedObjectSizePaires.add(new MutablePair<String, Long>(object, objMeta.getContentLength() <= CosSplitUtil.SINGLE_FILE_SPLIT_THRESHOLD_IN_SIZE ? -1L : objMeta.getContentLength()));
                        parsedObjectSizePaires.add(new MutablePair<String, Long>(object, objMeta.getContentLength()));
            ...
            if (StringUtils.isNotBlank(errorMessage)) {
                if (errorMessage.contains("UnknownHost")) {
                    ...
                    if (e instanceof CosServiceException) {
                        CosServiceException ossException = (CosServiceException) e;
        ...
        private List<Pair<String, Long>> getAllRemoteObjectsKeyAndSizeInDir(String parentDir) {
            ...
            for (ObjectListing objectList : objectListings) {
                for (COSObjectSummary objectSummary : objectList.getObjectSummaries()) {
                    //
                    LOG.info("object key:{},size:{}", objectSummary.getKey(), objectSummary.getSize());
                    LOG.debug("getAllRemoteObjectsKeyAndSizeInDir()");
//                    Pair<String, Long> objNameSize = new MutablePair<String, Long>(objectSummary.getKey(), objectSummary.getSize() <= CosSplitUtil.SINGLE_FILE_SPLIT_THRESHOLD_IN_SIZE ? -1L : objectSummary.getSize());
                    Pair<String, Long> objNameSize = new MutablePair<String, Long>(objectSummary.getKey(), objectSummary.getSize());
        ...
        private List<ObjectListing> getRemoteObjectListings(String parentDir) {

            List<ObjectListing> remoteObjectListings = new ArrayList<ObjectListing>();

            LOG.debug("Parent folder: {}", parentDir);
            
            COSClient client = CosUtil.initOssClient(readerOriginConfig);
    ...
    public static class Task extends Reader.Task {
        ...
        public void startRead(RecordSender recordSender) {
            ...
            this.ossClient = CosUtil.initOssClient(this.readerSliceConfig);
            for (StartEndPair eachSlice : this.allWorksForTask) {
                ...
                LOG.info(String.format("read bucket=[%s] object=[%s], range: [start=%s, end=%s] start...", bucket,
                        object, start, end));
                InputStream ossInputStream = new CosInputStream(ossClient, bucket, object, start, end);
                // 检查是否要跳过表头, 防止重复跳过首行
